...
export function app(server){
      ...
      return getUser(recipientId).then(user => {
        ...
        if(typeof user == 'undefined'){
          return Business.findOne({where: {fanpageId: senderId}}).then((business)=>{
            ...
            return User.createUser(store, recipientId, recipientId, business.fanpageToken).then(()=>{
              let userObject = getData(recipientId, 'user');
              let user = userObject.rawParseObject;

              return user.createConsumer(store, recipientId, senderId, business.fanpageToken).then(consumer => {
                return new Promise((resolve, reject) => {
                  resolve(userObject)
                });
              });
  ...
  function getCustomer(recipientId, senderId){
    ...
    if(typeof customer == 'undefined'){
      return Business.findOne({where: {fanpageId: senderId}}).then((business)=>{
        return Customer.loadInStore(store, recipientId, business.id).then(customer => {

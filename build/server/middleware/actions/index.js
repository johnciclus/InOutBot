...
function loadProducts(senderId, lat, lng, category, pointSale) {
    return Business.findOne({ where: { fanpageId: senderId } }).then(function (business) {
        return function (dispatch, getState) {
            var params = new GetProductsParams_1.default(business.id);
            if (lat && lng) {
                params.lat = lat;
                params.lng = lng;
            }
...
function updateConsumer(senderId, consumerData) {
    return Business.findOne({ where: { fanpageId: senderId } }).then(function (business) {
        return function (dispatch) {
            ...
            consumer.save(consumerData).then(function (consumer) {
                var username = consumer.get('email') + business.id;
...
function emailLogin(senderId, userData, mainDispatch) {
    return Business.findOne({ where: { fanpageId: senderId } }).then(function (business) {
        ...
        return function (dispatch) {
            parse_1.default.User.logIn(userData.email + business.id, userData.password).then(function (user) {
                dispatch({
...
function emailRegister(senderId, userData, mainDispatch) {
    return Business.findOne({ where: { fanpageId: senderId } }).then(function (business) {
        ...
        return function (dispatch) {
            parse_1.default.User.signUp(userData.email + business.id, userData.password).then(function (user) {
                dispatch({
...
function loadPointSales(senderId) {
    return Business.findOne({ where: { fanpageId: senderId } }).then(function (business) {
        var params = { businessId: business.id };
        return function (dispatch) {
